...
def load_checkpoint(args):
    ...
    def get_models(tp_size, ep_size, dtype):
        ...
        for rank in range(tp_size):
            ...
            for e_rank in range(ep_size):
                mpu.set_expert_model_parallel_rank(e_rank)
                ...
                for vp_rank in range(model_array_len):
                    models[vp_rank].append(model_[vp_rank])  # [[t0e0, t0e1, ...]]
    ...
    hf_state_dict["model.embed_tokens.weight"] = torch.cat(
        [models[tp_rank].embedding.word_embeddings.weight.data for tp_rank in range(0, tp_size*ep_size, ep_size)],
        dim = 0).clone()
    ...
    for vp_rank in range(vp_size):
        ...
        for pp_rank in range(pp_size):
            ...
            for layer_num in range(len(get_transformer_block(models[0]).layers)):
                ...
                hf_layer_pre = f"model.layers.{layer_num}"
                hf_state_dict[f"{hf_layer_pre}.input_layernorm.weight"] = layer.self_attention.linear_qkv.layer_norm_weight.data.clone() + 1.0
                hf_state_dict[f"{hf_layer_pre}.post_attention_layernorm.weight"] = layer.pre_mlp_layernorm.weight.data.clone() + 1.0
                hf_state_dict[f"{hf_layer_pre}.block_sparse_moe.gate.weight"] = layer.mlp.router.weight.data.clone()

                # Grab all parallel tensors for this layer
                qkv_weight = []
                dense_weight = []
                mlp_w1_weight = [[] for _ in range(md.num_experts)]  # [[e1t1, e1t2, ...], [e2t1, e2t2, ...]]， each elemtn is a list of length tp_size, element's shape is (hidden_size, ffn_hidden_size / tp)
                mlp_w2_weight = [[] for _ in range(md.num_experts)]  # [[e1t1, e1t2, ...], [e2t1, e2t2, ...]]， each elemtn is a list of length tp_size, element's shape is (ffn_hidden_size / tp, hidden_size)
                mlp_w3_weight = [[] for _ in range(md.num_experts)]  # [[e1t1, e1t2, ...], [e2t1, e2t2, ...]]， each elemtn is a list of length tp_size, element's shape is (hidden_size, ffn_hidden_size / tp)
                for rank in range(0, tp_size*ep_size, ep_size):
                    model = models[rank]
                    ...
                    # Grab moe weights (have both tensor & expert parallel)
                    if md.moe_grouped_gemm and md.swiglu:
                        l0_all_experts, l1_all_experts = [], []
                        for e_rank in range(ep_size):
                            global_idx = rank + e_rank
                            experts = get_transformer_block(models[global_idx]).layers[layer_num].mlp.experts
                            l0_ungroup = experts.weight1.data.view(num_local_experts, md.hidden_size, -1)  # (num_local_experts, hidden_size, ffn_hidden_size / tp * 2)
                            l0_ungroup = torch.chunk(l0_ungroup, num_local_experts)  # num_local_experts * (1， hidden_size, ffn_hidden_size / tp * 2)
                            l1_ungroup = experts.weight2.data.view(num_local_experts, -1, md.hidden_size)  # (num_local_experts, ffn_hidden_size / tp, hidden_size)
                            l1_ungroup = torch.chunk(l1_ungroup, num_local_experts)  # num_local_experts * (1， ffn_hidden_size / tp, hidden_size)
                            for i, (l0_expert_i, l1_expert_i) in enumerate(zip(l0_ungroup, l1_ungroup)):
                                expert_idx = e_rank * num_local_experts + i
                                # convert l0 to w1 & w3, then gather tensor parallel
                                w1, w3 = torch.chunk(l0_expert_i.squeeze(0), 2, dim=1)
                                mlp_w1_weight[expert_idx].append(w1)
                                mlp_w3_weight[expert_idx].append(w3)
                                mlp_w2_weight[expert_idx].append(l1_expert_i.squeeze(0))
                    else:
                        raise Exception('Only implemented for moe_grouped_gemm==True and swiglu==True')

                # Handle gated linear units
                if md.moe_grouped_gemm and md.swiglu:
                    for expert_idx in range(md.num_experts):
                        w1 = mlp_w1_weight[expert_idx]  # tp_size * (hidden_size, ffn_hidden_size / tp)
                        w2 = mlp_w2_weight[expert_idx]  # tp_size * (ffn_hidden_size / tp, hidden_size)
                        w3 = mlp_w3_weight[expert_idx]  # tp_size * (hidden_size, ffn_hidden_size / tp)
                        # gather all tp
                        w1 = torch.cat(w1, dim=1).transpose(0,1)
                        w2 = torch.cat(w2, dim=0).transpose(0,1)
                        w3 = torch.cat(w3, dim=1).transpose(0,1)

                        hf_state_dict[f"{hf_layer_pre}.block_sparse_moe.experts.{expert_idx}.w1.weight"] = w1
                        hf_state_dict[f"{hf_layer_pre}.block_sparse_moe.experts.{expert_idx}.w2.weight"] = w2
                        hf_state_dict[f"{hf_layer_pre}.block_sparse_moe.experts.{expert_idx}.w3.weight"] = w3
                else:
                    raise Exception('Unimplemented')  # if not moe_grouped_gemm, then might not need to do the transpose
    ...
    def save_config():
        ...
        config.router_aux_loss_coef = margs.moe_aux_loss_coeff
...
def main():
    ...
    parser.add_argument('--ckpt-step', type=int, default=None, help='checkpoint step')
