...
from megatron.core.optimizer.optimizer import register_param_names_with_optimizer
...
def setup_model_and_optimizer(model_provider_func,
    ...
    optimizer = register_param_names_with_optimizer(model, optimizer)
...
def train_step(forward_step_func, data_iterator,
    ...
    if should_exit:
        # Return None for per_layer_grad_norms if exiting early
    ...
    # Determine if per-layer norms should be calculated this step
    # Check based on logging flag and frequency setting
    should_calculate = False
    if args.log_per_layer_grad_norm: # Only consider if feature is enabled
        if args.log_per_layer_grad_norm_freq == 'every_step':
            should_calculate = True # Calculate every step if requested
        elif args.log_per_layer_grad_norm_freq == 'interval':
            # Calculate only on log interval steps
            should_calculate = (args.curr_iteration % args.log_interval == 0) 
        # Add other frequency options here if needed in the future
    
    # Update parameters.
    timers('optimizer', log_level=1).start(barrier=args.barrier_with_L1_time)
    # Pass the determined flag to optimizer.step
    update_successful, grad_norm, num_zeros_in_grad, per_layer_grad_norms = optimizer.step( 
        calculate_per_layer_norms=should_calculate 
    ) 
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        ...
        # Return calculated norms (or None)
        return loss_reduced, skipped_iter, should_checkpoint, should_exit, exit_code, grad_norm, num_zeros_in_grad, per_layer_grad_norms 
    # Return calculated norms (or None)
    return {}, skipped_iter, should_checkpoint, should_exit, exit_code, grad_norm, num_zeros_in_grad, per_layer_grad_norms


def training_log(loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration,
                 loss_scale, report_memory_flag, skipped_iter,
                 grad_norm, params_norm, num_zeros_in_grad, per_layer_grad_norms=None):
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0):
        ...
        # Log per-layer gradient norms to wandb
        if wandb_writer and per_layer_grad_norms:
            if args.log_per_layer_grad_norm_freq == 'every_step' or (iteration % args.log_interval == 0):
                # Log each parameter's gradient norm
                layer_metrics = {}
                for param_name, norm_value in per_layer_grad_norms.items():
                    # Create a simpler key by keeping just the essential part
                    key = f"grad_norm/{param_name}"
                    layer_metrics[key] = norm_value
                
                wandb_writer.log(layer_metrics, iteration)
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        loss_dict, skipped_iter, should_checkpoint, should_exit, exit_code, grad_norm, num_zeros_in_grad, per_layer_grad_norms = \
        ...
        report_memory_flag = training_log(loss_dict, total_loss_dict,
                                          ...
                                          grad_norm, params_norm, num_zeros_in_grad,per_layer_grad_norms=per_layer_grad_norms)
