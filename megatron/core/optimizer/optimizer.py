...
def register_param_names_with_optimizer(model, optimizer):
    """Create a mapping from parameter objects to their full names and attach to optimizer.
    
    This function creates a mapping from parameter objects to parameter names, or two parameters in the case of mixed precision.
    
    These mappings are useful for debugging, tracking gradient norms by layer, and generating 
    human-readable logs. We use them primarily for tracking the gradient norms of each layer.
    
    Args:
        model: The model or list of models whose parameters we want to track
        optimizer: The optimizer that will be augmented with parameter name mappings
        
    Returns:
        optimizer: The same optimizer but with added parameter name mappings
    """
    # Create mapping dictionary from parameter objects to parameter names
    param_obj_to_name = {}
    
    # Handle case where model is a list of models (pipeline parallel, etc.)
    if isinstance(model, list):
        for i, model_chunk in enumerate(model):
            # Add prefix to distinguish parameters from different model chunks
            # This ensures unique naming when we have multiple model chunks
            prefix = f"model_chunk_{i}."
            for name, param in model_chunk.named_parameters():
                prefixed_name = prefix + name
                param_obj_to_name[param] = prefixed_name
    else:
        # For a single model, just map the parameter names directly
        for name, param in model.named_parameters():
            param_obj_to_name[param] = name

    # Helper 1: Add mappings for mixed precision params (fp16/bf16 and their fp32 copies)
    def _add_mixed_precision_mappings(optimizer_obj, target_dict, global_name_map):
        # These optimizers maintain parallel lists of original (e.g., fp16/bf16) 
        # and fp32 copy parameters.
        for model_group, main_group in zip(optimizer_obj.float16_groups, optimizer_obj.fp32_from_float16_groups):
            for model_param, main_param in zip(model_group, main_group):
                # Find the original name using the model_param key in the global map
                if model_param in global_name_map:
                    main_name = global_name_map[model_param]

                    # If model_param is not already a key in the target_dict, 
                    # it implies target_dict is a new dictionary (e.g., inner_param_obj_to_name 
                    # for a chained optimizer), so we need to add the model_param mapping.
                    if model_param not in target_dict:
                        target_dict[model_param] = main_name 
                        
                    # Always add the main_param mapping, as it might not exist in the initial 
                    # global map or the target_dict.
                    target_dict[main_param] = main_name

    # Helper 2: Populate a target map based on whether the optimizer uses mixed precision
    def _populate_map_for_optimizer(optimizer_obj, target_dict, global_name_map):
        # Check if the optimizer uses mixed precision
        if hasattr(optimizer_obj, 'fp32_from_float16_groups') and hasattr(optimizer_obj, 'float16_groups'):
            _add_mixed_precision_mappings(optimizer_obj, target_dict, global_name_map)
        else:
            # Standard optimizer, map params directly if they exist in the global map
            if hasattr(optimizer_obj, 'param_groups'):
                for group in optimizer_obj.param_groups:
                    for param in group['params']:
                        if param in global_name_map:
                            # Only add if not already present (relevant for chained case where target_dict starts empty)
                            if param not in target_dict: 
                                target_dict[param] = global_name_map[param]

    # --- Main Logic --- 
    if hasattr(optimizer, 'chained_optimizers'):
        # Iterate through the underlying chained optimizers
        for inner_optimizer in optimizer.chained_optimizers:
            if inner_optimizer is not None: # Ensure inner optimizer exists
                # Create a new subset mapping for this inner optimizer
                inner_param_obj_to_name = {}
                # Populate the map using the helper
                _populate_map_for_optimizer(inner_optimizer, inner_param_obj_to_name, param_obj_to_name)
                # Attach the populated subset mapping to the inner optimizer
                inner_optimizer.param_obj_to_name = inner_param_obj_to_name

    else: # Not a ChainedOptimizer
        # Populate the main map using the helper (modifies param_obj_to_name directly)
        _populate_map_for_optimizer(optimizer, param_obj_to_name, param_obj_to_name)
    
    # Attach the final param_obj_to_name map (potentially modified for single mixed-precision optimizers)
    optimizer.param_obj_to_name = param_obj_to_name
    return optimizer
...
class MegatronOptimizer(ABC):
    ...
    @abstractmethod
    def step(self, calculate_per_layer_norms: bool = False):
        """Step the optimizer.
        
        Args:
            calculate_per_layer_norms (bool): Whether to calculate and return per-layer norms.
        """
        pass
...
class MixedPrecisionOptimizer(MegatronOptimizer):
    ...
    def step(self, calculate_per_layer_norms: bool = False):
        ...
        if found_inf_flag:
            # Return None for norms if step fails
            return False, None, None, None 

        # TODO: move this into get_main_grads_for_grad_norm(self), so that we retrieve the per-layer grad norm and compute the total grad norm in one function.
        # --- Grad Norm Calculation ---
        grad_norm = None
        per_layer_grad_norms = None 

        # Calculate per-layer norms if requested
        if calculate_per_layer_norms:
            per_layer_grad_norms = {}
            # This section maps gradients to their corresponding parameter names for tracking layer-wise gradient norms
            # It relies on the parameter-to-name mappings created by register_param_names_with_optimizer()
            if not self.is_stub_optimizer:
                # param_obj_to_name provides a direct lookup from parameter objects to their full hierarchical names
                if hasattr(self, 'param_obj_to_name'):
                    for param_group_idx, param_group in enumerate(self.optimizer.param_groups):
                        for param_idx, param in enumerate(param_group['params']):
                            if hasattr(param, 'grad') and param.grad is not None:
                                # First try to use the name from the parameter object mapping 
                                if param in self.param_obj_to_name:
                                    param_name = self.param_obj_to_name[param]
                                else:
                                    # Fall back to a generic name if the parameter wasn't registered with a name
                                    param_name = f"group_{param_group_idx}_param_{param_idx}"
                                
                                # Store the gradient norm with the parameter name as key
                                per_layer_grad_norms[f"grad_norm/{param_name}"] = param.grad.norm().item()
                else:
                    # If no param_obj_to_name mapping exists, use generic names based on indices
                    for param_group_idx, param_group in enumerate(self.optimizer.param_groups):
                         for param_idx, param in enumerate(param_group['params']):
                            if hasattr(param, 'grad') and param.grad is not None:
                                key = f"group_{param_group_idx}_param_{param_idx}"
                                per_layer_grad_norms[f"grad_norm/{key}"] = param.grad.norm().item()
        ...
        #TODO: move sanity check
        # Sanity check for gradient norm calculation
        if calculate_per_layer_norms and per_layer_grad_norms:
            # Calculate L2 norm from per-layer float norms
            sum_sq_norms = sum(norm**2 for norm in per_layer_grad_norms.values() if isinstance(norm, (float, int)))
            computed_norm_float = math.sqrt(sum_sq_norms)
            
            # Get float value of aggregate grad_norm (handle tensor or float)
            grad_norm_float = grad_norm.item() if isinstance(grad_norm, torch.Tensor) else grad_norm

            # Compare the float values
            if not math.isclose(grad_norm_float, computed_norm_float, rel_tol=1e-3, abs_tol=1e-5):
                logger.warning(
                    f"Gradient norm mismatch: Aggregate norm ({grad_norm_float:.4f}) vs. "
                    f"Computed norm from layers ({computed_norm_float:.4f})"
                )
        # Successful update. Return norms if calculated, else None.
        return success, grad_norm, num_zeros_in_grad, per_layer_grad_norms
...
class FP32Optimizer(MegatronOptimizer):
    ...
    def step(self, calculate_per_layer_norms: bool = False):
        ...
        # FP32 optimizer doesn't calculate per-layer norms in this flow
        per_layer_grad_norms = None 
        ...
        # No overflow for FP32 optimizer. Return None for norms.
        return success, grad_norm, num_zeros_in_grad, per_layer_grad_norms
...
class ChainedOptimizer(MegatronOptimizer):
    ...
    def step(self, calculate_per_layer_norms: bool = False):
        ...
        Args:
            calculate_per_layer_norms (bool): Whether to calculate gradient norms for each layer.
        
        Returns:
            tuple: A tuple containing:
                - success (bool): Whether the step was successful.
                - total_norm (float or None): The total gradient norm, or None if not calculated.
                - num_zeros (int or None): The number of zero gradients, or None if not calculated.
                - per_layer_grad_norms (dict or None): A dictionary of per-layer gradient norms, or None if not calculated.
        """
        if self.is_stub_optimizer:
            # Return None for norms
        ...
        # TODO: move this into get_main_grads_for_grad_norm(self), so that we retrieve the per-layer grad norm and compute the total grad norm in one function.
        per_layer_grad_norms = None
        # --- Conditional Per-Layer Grad Norm Calculation --- 
        if calculate_per_layer_norms:
            per_layer_grad_norms = {}
            # This section maps gradients to their corresponding parameter names for tracking layer-wise gradient norms
            # It relies on the parameter-to-name mappings created by register_param_names_with_optimizer()
            for optimizer_idx, optimizer in enumerate(self.chained_optimizers):
                # Check if the inner optimizer has the name map attached
                if hasattr(optimizer, 'param_obj_to_name'):
                    for param_group_idx, param_group in enumerate(optimizer.param_groups):
                        for param_idx, param in enumerate(param_group['params']):
                            if hasattr(param, 'grad') and param.grad is not None:
                                # First try to use the name from the parameter object mapping
                                if param in optimizer.param_obj_to_name:
                                    param_name = optimizer.param_obj_to_name[param]
                                else:
                                    # Fall back to a generic name if the parameter wasn't registered with a name
                                    param_name = f"opt_{optimizer_idx}_group_{param_group_idx}_param_{param_idx}"
                     ...
                     for param_group_idx, param_group in enumerate(optimizer.param_groups):
                        for param_idx, param in enumerate(param_group['params']):
                            if hasattr(param, 'grad') and param.grad is not None:
                                key = f"opt_{optimizer_idx}_group_{param_group_idx}_param_{param_idx}"
                                per_layer_grad_norms[f"grad_norm/{key}"] = param.grad.norm().item()
        # --- End Conditional Calculation --- 
        ...
        # Return norms if calculated, else None
        return update_successful, grad_norm, num_zeros_in_grad, per_layer_grad_norms
